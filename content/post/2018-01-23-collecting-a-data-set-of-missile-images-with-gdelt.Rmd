...
Let's see how well we can scoop up a set of missile images, ballistic missiles specifically, using a combination of packages:

* `gdeltr2`: a package that is decidedly buggy, but works for our purposes
* `tidyverse` suite:
    + `dplyr` for data carpentry
    + `stringr` for string processing
    + `purrr` for functional enhancements and simple error-handling
...
We'll `pull()` all the languages from `code_book = "languages"` into a `vector` variable we can use as a search argument like so:
...
* generalized _Surface-to-`"air"`_ doesn't seem "loose" enough to ever reference ballistic missiles
...
```{r}
# setwd("C:/Users/Windows/Documents/syknapptic/")
...
2. select 100 random rows `sample_n(100)`
3. `pull()` the `urlImage` column into a vector
4. iterate (`walk()`) through each item of the vector
    + `safe_download()` each image's binary format (`mode = "wb"`)
        + and write it to `dir` using its `valid_path_regex`
...
Clearly the results are not perfect. There are images without anything resembling a missile as well as near duplicate images. That said, we can manually name files in such a way to omit those that are not useful.
