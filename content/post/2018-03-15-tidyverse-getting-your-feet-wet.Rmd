...
First,I saw a bit of R code written on a campus whiteboard that had nothing to do with myself, but was straight out of the workshop. It may have come from some of my data-centric colleagues who use R, so I didn't read too far into it.

Then, I overheard a conversation involving R from those in a program that doesn't require any data-related coursework. Many folks are familiar with the name as the school's primary data analysis course uses the `{Rcmdr}` GUI for statistical analysis, but these students would not have necessarily taken the course. I was cautiously optimistic.

Finally, a student who didn't even attend came to my office hours asking for resources. Why exactly? Some of his work colleagues attended. Apparently, they are now trying to incorporate some R-powered analysis in their work and he doesn't want to miss out.

The workshop consisted of 3 consecutive Fridays lasting 90 minutes each. That's only a total of 4.5 hours.

Even in that short amount of time, a gentle introduction to R was enough to convince participants (and non-participants!) to take advantage of the power that a bit of data-centric coding can offer.

# Reflection

I taught a similar 90 minute workshop last spring using R, but focussed on data types. 10 minutes in, I'm trying to explain the difference between `data.frame` and a `matrix` and the person asking the question says something along the lines of "I guess I'm kinda dumb. Don't worry about it."

For context, these were international policy graduate students. While some have completed a bit of quantitative coursework, most don't have a hardcore math or science background and programming is seen as something akin to wizardry. That said, they hold domain expertise in some rather important subjects. These include WMD nonproliferation, international development, economic diplomacy, conflict studies, and environmental policy. Nearly half of the participants were international students and everyone holds proficiency in minimally two natural languages. Most have already tackled big, critical problems in their careers and the others are on their way to doing so following graduation. In a nutshell, they're not dumb. The way I was teaching was dumb. After the workshop, I never heard anyone mention R outside my circle of fellow data folks.

Since that time, I started using R more. _Like, a lot more_. I was still avoiding the strange "tidy" code I'd encounter on Stack Overflow, but I realized the error of my ways when I came across Julia Silge and David Robinson's [Text Mining with R]("https://www.tidytextmining.com/"). It was like discovering that you're still in the stone age while most people were off partying on spaceships.

With that in mind, I knew if I ever had a chance to teach a bit of R code again, I would do so differently. The more code I wrote and the more insights from others I read, the more it became pretty obvious that `tidyverse` first makes sense.
...
If I could go back in time, it would have been awesome to have participants fill out surveys after the `base` workshop and after the `tidyverse` workshop for a less anecdotal comparison. Regardless, the level of engagement and type of response could not be more different.

<br>

Another huge takeaway is that __live coding works__.
...
The strategy is similar to many I've seen in the [#Rstats](https://twitter.com/search?q=%23rstats) community: introduce __useful__ tools immediately and reinforce them through multiple passes in slightly different contexts. 
