---
title: 'CSV to Data Frame: Establishing a Meaningful Standard for Performance Comparisons'
draft: false
...
Most seem to be run in Jupyter Notebook and many are using Python's `rpy2` library to run poorly optimized R code. I'm not an anti-`for()` loop Nazi (yes, you _can_ use them effectively in R), but thanks to the `base::*apply()` family and their beautiful `purrr::map*()` children, you'll rarely see explicit loops that actually provide the best solution. Unfortunately, some of these comparisons arbitrarily test loops in R where you would _never, ever_ do so. 

In a world of vectorization, it doesn't make any sense why code like this receives such prominent treatment in seemingly every test.....

```{r}
some_stuff <- rnorm(2500)
other_stuff <- vector(mode = "numeric", length(some_stuff))

for(i in some_stuff) {
  other_stuff[i] <- some_stuff[i] * some_stuff[i]
}
```

```{r, echo=FALSE}
knitr::include_graphics("https://media.giphy.com/media/l4FGnnlIQslHkOPaU/giphy.gif")
```

In R, just do this...

```{r}
better_stuff <- some_stuff^2

identical(other_stuff, better_stuff)
```

... and in Python, using `reticulate` here, we can do this...

```{r}
py_run_string(
"some_py_stuff = r.some_stuff
other_py_stuff = {}", convert = FALSE)

py_loop <- (
"for i in range(len(some_py_stuff)):
  other_py_stuff[i] = some_py_stuff[i] * some_py_stuff[i]"
)
```

... but the "Good R" code leaves it in the dust.

```{r}
mark(
  for(i in some_stuff) other_stuff[i] <- some_stuff[i] * some_stuff[i],
  some_stuff^2,
  py_run_string(py_loop, convert = FALSE),
  ...
  mutate_at(vars(type, expression), funs(
    cell_spec(., color = if_else(median == min(median), "green", "black"))
    )) %>% 
  select(type, expression, min, median, max) %>% 
...
There are problems with this test. Are we _really_ testing the same thing? Is running the Python code through an R's `reticulate` _actually_ fair? 

Well, is it less fair than, say, running `rpy2` in Python? After running all these tests, I'd certainly say it's more fair.

While some of these articles notify readers that the tests are lacking, some will sell the results as gospel anyways. Others dismiss the merits of rigor entirely. Le sigh.

It's concerning that, in a field referred to as "Data Science", there are a mountain of articles discussing such poor metrics, along with plenty of practitioners that use them in explaining which language newcomers should learn. (The answer is both, BTW)
...
With that in mind, what _would_ an objective comparison look like?

Here's a barrage of tests applied to a task that's both a) common in practice and b) common in these "language war" tests: reading a .csv file to a data frame. This is a task where many articles assert Python's superiority, despite the evidence here and elsewhere.

However, the __real__ goal is to experiment with methods that can be used to make tests involving less trivial tasks more objective _and more useful to everyone_.

That said, I thnk it's a cool demonstration of some RStudio and {`reticulate`} sweetness. Hopefully, this spurs some interest in how awesome a multilingual workflow can be.

If you want skip a pile of monotonous code, go ahead and jump to the [results](#execution-times).
...
At 5,000,000 million rows, we've reached the area where time differences would _actually_ be noticeable.
...
## Controlling for Dependency Load Times
